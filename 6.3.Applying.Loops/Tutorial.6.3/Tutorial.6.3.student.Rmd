...
First, create an empty matrix (every value is NA), with 5 rows and 6 columns. Row names should be the values in the `diseases` vector, and column names are the 6 possible values for the `clinical.significance` column (provided in the vector `significance`). Call the matrix "variant_significance_by_disease".
...
Write nested for loops to fill the values in the `variant_significance_by_disease` matrix. The matrix should be filled such that the value at `variant_significance_by_disease["type.2.diabetes", "Benign"]` is the number of variants associated with type 2 diabetes mellitus that have "Benign" clinical significance.
...
Write nested for loops to loop through *positional* row and column indices of the `variant_significance_by_disease` matrix and divide the element by the number of disease variants for the appropriate disease. Replace each element in `variant_significance_by_disease` with the new value.
Hint: Loop through the diseases (row indices) in the outer loop and clinical significance categories (column indices) in the inner loop. You may want to include a line of code wihtin the outer loop that is not in the inner loop.
...
The rest of the questions in this section will not use the ClinVar data.

The factorial of n is the product of all positive integers from 1 to n. For example, 4 factorial is 1 x 2 x 3 x 4 = 24.
Use nested loops to fill the vector `factorials` with the factorial of 1 to 10. One loop should be a for loop, and one should be a while loop.
...
```{r}
. = ottr::check("tests/Tutorial.W6.3.2.a.R")
```
**Check your answer by running the above block** 
...
Write code to determine the number of benign or likely benign variants in the `clinvar_data` data frame by vectorizing the above code. Save the number of benign or likely benign variants to a variable called "num_benign_var_2".

Hint: The number of `TRUE` values in a logical vector can be counted using the `length()` and `which()` functions:

    length(which(logical vector))
    
Is there a way to get a logical vector indicating which elements in `clinvar_data$clinical.significance` are "Benign" or "Likely_benign"?
...
Hint: Is there a way to reduce `clinvar_data` to only the "chromosome" and "gene" columns and to get all the unique rows?
...
Use nested for loops to create a list called "pathogenic_gene_variants". Each list element should contain the number of pathogenic or likely pathogenic variants in each gene in `clinvar_data`. The name of the element should be the gene name. The outer for loop should iterate through the unique genes in `clinvar_data`. 
