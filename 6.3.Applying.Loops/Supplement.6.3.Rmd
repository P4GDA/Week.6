...
In the first iteration of the outer loop, `num_1` is equal to 1. While `num_1` is equal to 1, the inner loop runs. In the first iteration of the inner loop, `num_2` is equal to 5, and then `num_2` is equal to `10`. When this loop finishes, the outer loop will move on to the next value of `num_1`.
...
Let's loop through each row and column in the matrix and change the value of the element so that it is the product of the row and column indices:
...
If we look at the output, we can see that in the first iteration, the value at row 1, column 1 was changed to 1. In the second iteration, the value at row 1, column 2 was changed to 2. If you continue to look at output you can see that each row was completed column by column, because the row indices are in the outer loop.

--- 

We will again look at examples using brain tissue expression data sourced from the Human Protein Atlas (https://www.proteinatlas.org/humanproteome/brain):
...
Let's create a matrix of the correlation of expression between all 10 brain tissues. We will start by creating an empty matrix with 10 rows and 10 columns, which each value set to NA. The names of the 10 tissues are added as row and column names:
...
In the 6.1 Lecture Supplement we calculated the correlation in gene expression between the amygdala and each of the other brain tissues (including itself). This time we will do what we did for the amygdala, but this time for all pairing of all 10 tissues. We can use nested for loops, first looping through the row names of the matrix and then the column names. Within each iteration we can calculate the expression correlation between the two tissues, round it, and add the value to our `tissue_expr_cor` matrix:
...
In the 6.2 Lecture Supplement we calculated the minimum number of genes that accounted for 50% of the RNA expression in the midbrain, and used that value to determine the percentage of genes that accounted for 50% of the RNA. How could we calculate the percentage of genes the accounted for 50% of the expression for all 10 tissues? 

We could use a for loop to go through the tissues, and a while loop to get the number of genes. Within the for loop, we will sort expression values for the current tissue, determine half of the expression, and use the same nested while loop that we used in the 6.2 Lecture Supplement to calculate the number of genes. After the while loop terminates, we can calculate and add the percentage to our vector within the for loop.

In previous nested loops we did not include any code in between the declaration of the outer and inner loops, however we can write code that occurs for each item in the outer loop before running the inner loop:
...
When we look at the output we can see that for all of the tissues, around 19% of genes account for half of all expression.
...
In the 6.1 Lecture Supplement we looked at a real-life example that is similar to a for loop: a scientist is working in the lab with 6 samples, and for each sample the scientist needs to add 10 uL of lysis buffer and then place the sample on ice:

    For X in sample 1, sample 2, sample 3, sample 4, sample 5, sample 6:
      - Add 10 uL of lysis buffer to X
      - Place X on ice

In this example, the biologist does one sample and then moves onto the next. But what if all the tasks could be performed simultaneously? In this example there are two possible solutions:
- six scientists could perform the task, each performing the task for one sample
- one scientist using a multichannel pipette adds the buffer to all of the samples at the same time
...
Vectorized code is much more efficient and concise than non-vectorized code. Let's compare non-vectorized vs vectorized code to take the natural log of the numbers 1 through 4, and adding them to a vector `y`.
...
It is clear that this code is more concise, but it is also faster? To time how long a piece of code takes to run we can use the `Sys.time()` function, which outputs the current time when it is executed.

First, let's calculate how long the non-vectorized code (for loop) takes to run:
...
The vectorized code ran 3.22x faster. This may not seem important as both run times are less than 1 second, but when we are working with much larger vectors this becomes very important.
...
There is also a way to write your own vectorized functions. While we will not be focusing on the method in this course, it will be briefly covered in the 7.1 supplement.
...
This works, however the function `colMeans()` calculates the mean for each column in a data frame or matrix automatically. We can vectorize the above code by using the `colMeans()` function instead:
...
More examples of vectorization can be shown using this data set of containing genes along with the chromosome each gene is on and the strand. The data frame contains the following columns:
...
However, it can also easily be vectorized by using the `nrow()` and `filter()` functions:

```{r}
nrow(filter(genes_and_chromosomes, Chromosome == "X" | Chromosome == "Y"))
```

For each of the chromosomes in our data frame, how many genes does it have on the positive strain? Using nested for loops we can create a list called "chromosome_pos_genes", with each list element having the name of a chromosome and containing the number of positives strand genes on that chromosome. 
...
The inner loop in the above code can be vectorized using the same technique as we used when getting the number of genes on the X and Y chromosomes: `nrow(filter())`
