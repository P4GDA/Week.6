...
What will the value of `i` be when the while loop stops (doesn't complete any more iterations)? Assign the value to a variable called "i_final_value".
...
Split the RNA sequence in `RNA_seq` into codons using a while loop and add each codon to the vector `codons` below. 

Hints: Use the `nchar()` function to make the loop work for any length of RNA sequence (not just the one below). If You should also use the `substr()` function. The first codon can be retrieved using the code: `substr(1, 3, RNA_seq)`. The stopping condition of the loop should involve making sure your values for extracting codons do not go past `nchar(RNA_seq)`.
